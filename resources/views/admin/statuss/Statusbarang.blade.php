        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form class="modal-content" action="ubahlelang/pemanang/{{$key->id_lelang}}" method="POST">
                ...
                <div class="col-8 ms-5 mb-3 form-group">
                    ...
                    <select class="form-control" id="id_user" aria-label="id_user" name="id_user">
                        <option class="col-sm-1" selected>Pilih Pemenang</option>
                        @php 
                        $contactss = \App\Models\historymodel::where('id_lelang', $key->id_lelang)
                            ->with('barang','lelang','masyarakat')->get();
                        @endphp

                       @foreach ($contactss as $menang)
                        <option value="{{ $menang->id_user}}">{{ $menang->id_user }} -> {{$menang->masyarakat->nama_lengkap}} -> @rupiah($menang->penawaran_harga)</option>
                        @endforeach
                    </select>
                </div>

                <div class="col-8 ms-5 mb-3 form-group">
                    <label class="col-form-label" for="harga_ahkir">Pilih Harga yang Ditentukan</label>
                    <select class="form-control" id="harga_ahkir" aria-label="harga_ahkir" name="harga_ahkir">
                        <option class="col-sm-1" selected>Pilih Harga</option>
                       @foreach ($contactss as $keym)
                        <option value="{{ $keym->penawaran_harga}}">{{$keym->masyarakat->nama_lengkap}} -> @rupiah($menang->penawaran_harga)</option>
                        @endforeach
                    </select>
                </div>
